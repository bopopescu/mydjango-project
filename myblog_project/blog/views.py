...
def post_detail(request, year, month, day, post):
    post_object = get_object_or_404(Post, slug=post)
                             # status='published',
                             # publish__year=year,
    ...
    similar_posts = Post.published.filter(tags__in=post_tags_ids).exclude(id=post_object.id)
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'yosubzzang@naver.com',[cd['to']])
            sent = True
            #send email
    else:
        form = EmailPostForm()
